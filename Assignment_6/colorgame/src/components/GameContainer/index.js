...
class GameContainer extends Component {
  ...
  createGrids(val) {
    for (let i = 1; i <= val; i++) {
      this.element.push(
        <div className="row">{this.createRow(this.count, val * i)}</div>
      );
    ...
    for (let i = start; i < end; i++) {
      if (this.state.indexArray.indexOf(i + 1) != -1)
        this.row.push(
          <GridBox
            value={i}
            key={this.state.level + "-" + i}
            cssStyle="clickedBox"
            array={this.state.indexArray}
            handlerFromParentSuccess={this.successEvent}
          />
        );
      else {
        this.row.push(
          <GridBox
            ...
            cssStyle="box"
            array={this.state.indexArray}
            handlerFromParentFail={this.failedEvent}
          />
        );
      }
    }
      ...
      <div className={this.state.fullViewOn + " fullView"}>
        <div className="container">
          <div>
            ...
            <div
              className={"parent " + this.state.parentCss}
